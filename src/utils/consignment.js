...
function normalizeRateType(value) {
  ...
  if (normalized === "cost_per_mt" || normalized.includes("mt"))
    return "cost_per_mt";
  return "fixed_cost";
}

function rateTypeLabel(value) {
  return normalizeRateType(value) === "cost_per_mt"
    ? "Cost Per MT"
    : "Fixed Cost";
}
...
function paymentTypeValue(value) {
  const normalized = String(value || "")
    .toLowerCase()
    .replaceAll(" ", "_");
  if (normalized === "truck_owner" || normalized === "owner_payment")
    return "Truck_Owner";
...
export function calculateConsignmentValues(form = {}) {
  const supplierRateType = normalizeRateType(
    form.supplierRateType ?? form.freightAmountType,
  );
  const supplierAmount = Number(form.supplierAmount || 0);
  const chargeableWeight = Number(
    form.chargeableWeight ?? form.chargebleWeight ?? 0,
  );
  const profitFixedCost = Number(
    form.profitFixedCost ?? form.customerAmount ?? 0,
  );
  const profitCostPerMT = Number(
    form.profitCostPerMT ?? form.customerAmount ?? 0,
  );
  const profitChargeableWeight = Number(
    form.profitChargeableWeight ?? form.customerChargeableWeight ?? 0,
  );
  ...
  const totalAdvance = Array.isArray(form.advanceEntries)
    ? form.advanceEntries.reduce(
        (sum, item) => sum + Number(item.amount || 0),
        0,
      )
    : Number(form.totalAdvance || 0);

  const baseFreightAmount =
    supplierRateType === "cost_per_mt"
      ? supplierAmount * chargeableWeight
      : supplierAmount;
  ...
  const calculatedFreightBookingCost =
    supplierRateType === "cost_per_mt"
      ? profitCostPerMT * profitChargeableWeight
      : profitFixedCost;
...
export function buildConsignmentPayload(form) {
  ...
  return {
    ...
    bookingDate: dateOrNull(
      calculatedForm.ledgerDate ?? calculatedForm.bookingDate,
    ),
    ...
    deliveryDate: dateOrNull(
      calculatedForm.deliveryDateTime ?? calculatedForm.deliveryDate,
    ),
    ...
    weightUnit: calculatedForm.weightUnit ?? "MT",
    ...
    profitCostPerMT:
      supplierRateType === "cost_per_mt"
        ? numberField(calculatedForm, "profitCostPerMT")
        : 0,
    profitChargeableWeight:
      supplierRateType === "cost_per_mt"
        ? numberField(calculatedForm, "profitChargeableWeight")
        : 0,
    freightBookingCost: numberField(calculatedForm, "customerRate"),
    additionalCharges: numberField(calculatedForm, "additionalCharges"),
    otherExpenses: numberField(calculatedForm, "otherExpenses"),
...
export function normalizeConsignment(item) {
  ...
  return applyCalculatedConsignmentValues({
    ...
    advanceEntries: Array.isArray(advanceEntriesSource)
      ? normalizeAdvanceEntries(advanceEntriesSource)
      : item.advanceEntries,
  });
}
