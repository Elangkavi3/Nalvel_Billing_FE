...
function lrRecordToForm(record, currentForm = {}) {
  ...
  return {
    ...
    insuranceNote:
      String(record?.customerInsured || "").toLowerCase() === "yes"
        ? "Insurance Covered by Customer"
        : "Insurance at Owners Risk",
...
async function fetchLrByConsignmentId(consignmentId) {
  ...
  return (
    lrRecords.find(
      (record) => resolveRecordConsignmentId(record) === targetId,
    ) || null
  );
}
...
async function saveLRToBackend(form) {
  async function resolveExistingLrRecordId() {
    ...
    try {
      const lrRecords = await fetchAllLrRecords();
      const serialNo = String(form.sourceSerialNo || "")
        .trim()
        .toLowerCase();
      const cnNo = String(form.lrNo || "")
        .trim()
        .toLowerCase();
      const matched = lrRecords.find((record) => {
        const recordConsignmentId = resolveRecordConsignmentId(record);
        const recordSerial = String(record?.savedDataSNo || "")
          .trim()
          .toLowerCase();
        const recordCno = String(record?.cnNo || "")
          .trim()
          .toLowerCase();
        return (
          (consignmentId && recordConsignmentId === consignmentId) ||
  ...
  const payload = {
    ...
    customerInsured: String(form.insuranceNote || "")
      .toLowerCase()
      .includes("covered")
      ? "Yes"
      : "No",
  ...
  const url = isUpdate
    ? lrEndpoints.updateById(targetLrRecordId)
    : lrEndpoints.create();
...
export function LRGenerationPage({ onBack, onSaved }) {
  ...
  async function loadFromSerial() {
    ...
    try {
      ...
      if (consignmentId) {
        try {
          ...
          if (record?.id) {
            ...
            setLookupMessage(
              `Loaded existing LR from backend for S.No ${prefill.savedDataSNo || serialNo}`,
            );
            return;
          }
        } catch {
      ...
      setForm((current) => ({
        ...
        consignorAddress:
          prefill?.consignorAddress ?? current.consignorAddress ?? "",
        from:
          current.from ||
          prefill?.fromLocation ||
          prefill?.consignorAddress ||
          "",
        consignorGstin: prefill?.consignorGstin ?? current.consignorGstin ?? "",
        consigneeName: prefill?.consigneeName ?? current.consigneeName ?? "",
        consigneeAddress:
          prefill?.consigneeAddress ?? current.consigneeAddress ?? "",
        to:
  ...
  async function loadSavedLrFromTable() {
    ...
    try {
      ...
      setForm((current) =>
        lrRecordToForm(matchedRecord, {
          ...current,
          sourceSerialNo: matchedRecord?.savedDataSNo || serialNo,
        }),
      );
      setLookupMessage(`Loaded saved LR #${matchedRecord.id} from LR table`);
    } catch (error) {
      setLookupMessage(
        error?.message || "Unable to load saved LR from LR table",
      );
    }
  }
  ...
  async function handleSubmit(event) {
    ...
    if (!form.consignmentId) {
      setLookupMessage(
        "Please load a saved entry using S.No before Save & Generate CN",
      );
      return;
    }
    ...
    try {
      ...
      onSaved?.(
        `LR ${form.lrNo || form.sourceSerialNo || ""} saved to backend successfully`,
      );
  ...
  async function handleDeleteLr() {
    ...
    if (!targetLrRecordId && form.consignmentId) {
      try {
        targetLrRecordId =
          firstLrRecord(await fetchLrByConsignmentId(form.consignmentId))?.id ||
          "";
      } catch {
        targetLrRecordId = "";
      }
  ...
  return (
    <section className="lr-page">
      <div className="lr-workspace">
        <form
          className="lr-form-panel lr-ui-page lr-edit-form"
        ...
        >
          <div className="lr-lookup-strip">
            <div className="lr-lookup-field">
              ...
              <input
                ...
                value={form.sourceSerialNo}
                onChange={(event) =>
                  updateField("sourceSerialNo", event.target.value)
                }
              />
            </div>
            <button
            ...
            <button
              type="button"
              className="btn btn-secondary"
              onClick={loadSavedLrFromTable}
            >
              Load Saved LR
            </button>
            {lookupMessage && (
              <div className="lr-lookup-message">{lookupMessage}</div>
            )}
          </div>
          ...
          <div className="lr-ui-header">
            ...
            <div className="company-info">
              ...
              <p>
                New No.12, Old No.26, Nallappa Street, Nehru Nagar, Chromepet,
                Chennai - 600 044
              </p>
              <p>
                Phone: 044-45830699 | nalvellogisticsservices@gmail.com | GSTIN:
                33ARXPK1573A2ZT
              </p>
            </div>
            <div className="badge-box">
              ...
              <div className="insurance-toggle-top">
                <span>Insured?</span>
          ...
          <div className="form-body">
            <div className="row">
              ...
              <div className="cell cell-date">
                ...
                <input
                  ...
                  value={form.lrDate}
                  onChange={(event) =>
                    updateField("lrDate", event.target.value)
                  }
                />
              </div>
            ...
            <div className="row">
              <div className="cell">
                ...
                <div className="two-col mt6">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        updateField("consignorName", e.target.value)
                      }
                    />
                  </div>
                  <div>
                    <div className="cell-label">GSTIN</div>
                    <input
                      className="field-input"
                      value={form.consignorGstin}
                      onChange={(e) =>
                        updateField("consignorGstin", e.target.value)
                      }
                    />
                  </div>
                </div>
                <div className="mt6">
                  ...
                  <textarea
                    className="field-input no-resize"
                    rows={2}
                    value={form.consignorAddress}
                    onChange={(e) =>
                      updateField("consignorAddress", e.target.value)
                    }
                  />
                </div>
              </div>
                ...
                <div className="two-col mt6">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        updateField("consigneeName", e.target.value)
                      }
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        updateField("consigneeGstin", e.target.value)
                      }
                    />
                  </div>
                </div>
            ...
            <div
              className={`row insurance-print-row${isCustomerInsured ? "" : " insurance-print-row-hidden"}`}
            >
              <div className="cell cell-flex2 insurance-full-row">
                {isCustomerInsured && (
                  <>
                    ...
                    <div className="three-col mt6">
                      <div>
                        <div className="cell-label">Company</div>
                        <input
                          className="field-input"
                      ...
                      <div>
                        <div className="cell-label">Policy No.</div>
                        <input
                          className="field-input"
                          value={form.policyNo}
                          onChange={(e) =>
                            updateField("policyNo", e.target.value)
                          }
                        />
                      </div>

                      <div>
                        <div className="cell-label">Policy Date</div>
                        <input
                          type="date"
                          className="field-input"
                          value={form.policyDate}
                          onChange={(e) =>
                            updateField("policyDate", e.target.value)
                          }
                        />
                      </div>
                    </div>
                    ...
                    <div className="two-col mt6">
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            updateField("insuranceAmount", e.target.value)
                          }
                        />
                      </div>

                      <div>
                        <div className="cell-label">Risk Type</div>
                ...
                {!isCustomerInsured && (
                  <div className="insurance-print-spacer" aria-hidden="true" />
                )}
              </div>
            </div>
              ...
              <div className="cell cell-flex3 package-details-cell">
                ...
                <table className="pkg-table mt6">
                  <thead>
                    <tr>
                      ...
                      <th className="pkg-col-description">
                    ...
                    <tr>
                      <td className="pkg-weight-cell">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            updateField("chargedWeight", e.target.value)
                          }
                        />
                      </td>
                    </tr>
                ...
                <div className="mt6 package-declared-value">
                  <div className="cell-label">
                    Declared Value (Rs) - * As per the Bill Attached.
                  </div>
                  <input
                    className="field-input mt4"
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td>
                        <input
                          ...
                          onChange={(e) =>
                            updateField("surcharge", e.target.value)
                          }
                        />
                      </td>
                    </tr>
                    <tr>
                      ...
                      <td>
                        <input
                          value={form.hamali}
                          onChange={(e) =>
                            updateField("hamali", e.target.value)
                          }
                        />
                      </td>
                    </tr>
                    <tr>
                      ...
                      <td>
                        <input
                          value={form.escort}
                          onChange={(e) =>
                            updateField("escort", e.target.value)
                          }
                        />
                      </td>
                    </tr>
                    <tr>
                      ...
                      <td>
                        <input
                          value={form.bocdd}
                          onChange={(e) => updateField("bocdd", e.target.value)}
                        />
                      </td>
                    </tr>
                    <tr>
                      ...
                      <td>
                        <input
                          value={form.stCharges}
                          onChange={(e) =>
                            updateField("stCharges", e.target.value)
                          }
                        />
                      </td>
                    </tr>
          ...
          <div className="btn-row">
            ...
            <a
              ...
              href={form.recipientEmail ? mailHref : undefined}
            >
              Send Email
            </a>
          ...
          <section className="lr-terms-print" aria-label="Terms and conditions">
            ...
            <div className="lr-terms-print-footer">
              All disputes subject to Chennai jurisdiction only. | This is a
  ...
  return (
    <form className="lr-form-panel lr-ui-page lr-print-duplicate">
      <div className="lr-ui-header">
        ...
        <div className="badge-box">
          <p>
            <strong>VEHICLE No.</strong> {form.vehicleNo || "-"}
          </p>

          <p>{form.contentsChecked || "CONTENTS NOT CHECKED"}</p>

          <p>{isCustomerInsured ? "Insured" : "Insurance at Owners Risk"}</p>
        </div>
      </div>
      ...
      <div className="form-body">
          ...
          <div className="cell cell-flex2 insurance-full-row">
            ...
            {isCustomerInsured && (
              <>
                ...
                <div className="two-col mt6">
                  <div>
                    <div className="cell-label">Amount (Rs)</div>
                    <input
                      className="field-input"
                      value={form.insuranceAmount}
                      readOnly
                  ...
                  <div>
                    <div className="cell-label">Risk Type</div>
                    <input className="field-input" value={form.risk} readOnly />
                  </div>
                </div>
              </>
            )}
        ...
        <div className="row">
          ...
          <div className="cell cell-flex12 freight-watermark-wrap">
            <div className="freight-watermark">
              As per the Bill
              <br />
              Attached
            </div>
            ...
            <table className="charge-table mt6">
              ...
              <tbody>
                <tr>
                  ...
                  <td>
                    <input value={form.freight} readOnly />
                  </td>
                </tr>
                <tr>
                  <td>Surcharge</td>
                  ...
                  <td>
                    <input value={form.surcharge} readOnly />
                  </td>
                </tr>
                <tr>
                  <td>Hamah</td>
                  ...
                  <td>
                    <input value={form.hamali} readOnly />
                  </td>
                </tr>
                <tr>
                  <td>Escort</td>
                  ...
                  <td>
                    <input value={form.escort} readOnly />
                  </td>
                </tr>
                <tr>
                  <td>BOD / DD</td>
                  ...
                  <td>
                    <input value={form.bocdd} readOnly />
                  </td>
                </tr>
                <tr>
                  <td>St. Charges</td>
                  ...
                  <td>
                    <input value={form.stCharges} readOnly />
                  </td>
                </tr>
                <tr className="total-row">
                  <td colSpan="2">TOTAL</td>
        ...
        <div className="pdf-border pdf-border-top">
          <div className="pdf-header">
            ...
            <div className="pdf-right-col">
              <div className="pdf-right-row">
                VEHICLE NO: {form.vehicleNo || "-"}
              </div>
              <div className="pdf-right-row">
                {form.contentsChecked || "CONTENTS NOT CHECKED"}
              </div>
              <div className="pdf-right-row">
                {form.insuranceNote || "Insurance at Owners Risk"}
              </div>
            </div>
          </div>
              ...
              <table>
                ...
                <tbody>
                  ...
                  <tr>
                    <td>St. Charges</td>
                    <td className="txt-center">-</td>
                    <td>{form.stCharges || "-"}</td>
                  </tr>
                  <tr className="total-row-pdf">
                    <td colSpan="2">TOTAL</td>
                    <td>{form.total || "-"}</td>
                  </tr>
                </tbody>
          ...
          {isCustomerInsured && (
            <div className="pdf-insurance">
              <b>INSURANCE DETAILS:</b> Company: {form.insuranceCompany || "-"}{" "}
              | Policy No: {form.policyNo || "-"} | Policy Date:{" "}
              {form.policyDate || "-"} | Amount: {form.insuranceAmount || "-"} |
              Risk Type: {form.risk || "-"}
            </div>
          )}
          ...
          <div className="pdf-footer">
            <div className="pdf-footer-left">
              {isCustomerInsured && (
                <div>
                  <b>Declared Value:</b> Rs. {form.invoiceValue || "-"}
                </div>
              )}
